...
class TransformerMatcher(object):
    ...
    def get_args_and_set_logger():
        ...
        parser.add_argument("--margin", default=1.0,
                            type=float, help="margin in hinge loss")
        parser.add_argument(
    ...
    def set_device(args):
        ...
        if args.no_cuda:
            ...
            if torch.cuda.is_available():
                logger.info(
                    'Local rank = -1 --> Setting device for training to available CUDA gpu')
            else:
                logger.info('Setting device for training to cpu')
    ...
    def predict(self, args, X_eval, C_eval_true, topk=10, get_hidden=False):
        ...
        args.eval_batch_size = args.per_device_eval_batch_size * \
            max(1, args.n_gpu)
        all_inst_idx = torch.tensor([f["inst_idx"]
                                     for f in X_eval], dtype=torch.long)
        all_input_ids = torch.tensor([f["input_ids"]
                                      for f in X_eval], dtype=torch.long)
        all_attention_mask = torch.tensor(
            [f["attention_mask"] for f in X_eval], dtype=torch.long)
        all_token_type_ids = torch.tensor(
            [f["token_type_ids"] for f in X_eval], dtype=torch.long)
        eval_data = TensorDataset(
            all_inst_idx, all_input_ids, all_attention_mask, all_token_type_ids)
        ...
        for batch in eval_dataloader:
            with torch.no_grad():
                ...
                c_eval = torch.tensor(
                    c_eval, dtype=torch.float).to(args.device)
                logger.info(
                    f'computing loss for c_pred of shape {c_pred.shape} and c_eval of shape {c_eval.shape}...')
            ...
            cpred_topk_rows = cpred_topk_rows.view(
                cur_batch_size, 1).expand_as(cpred_topk_cols)
    ...
    def train(self, args, X_trn, C_trn):
        ...
        args.train_batch_size = args.per_device_train_batch_size * \
            max(1, args.n_gpu)
        all_inst_idx = torch.tensor([f["inst_idx"]
                                     for f in X_trn], dtype=torch.long)
        all_input_ids = torch.tensor([f["input_ids"]
                                      for f in X_trn], dtype=torch.long)
        all_attention_mask = torch.tensor(
            [f["attention_mask"] for f in X_trn], dtype=torch.long)
        all_token_type_ids = torch.tensor(
            [f["token_type_ids"] for f in X_trn], dtype=torch.long)
        ...
        for epoch in range(1, int(args.num_train_epochs) + 1):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                labels = torch.tensor(
                    labels, dtype=torch.float).to(args.device)
...
def main():
    ...
    if args.do_eval:
        ...
        trn_csr_codes = transform_prediction(
            trn_csr_codes, transform="lpsvm-l2")
        ...
        tst_csr_codes = transform_prediction(
            tst_csr_codes, transform="lpsvm-l2")
